...
{
    public class ContractController : WebApiController
    {
        private readonly IContractTask _process;

        public ContractController(IContractTask process)
        {
            _process = process;
        }
        ...
        {
            int _rowcount = 0;
            var data = _process.Get(pageIndex, pageSize, out _rowcount, isActive);
            return Request.CreateResponse(HttpStatusCode.OK, new
            {
                data = new PagerRecord<ContractModel>(list: data,
                                                page: pageIndex,
                                                pageSize: pageSize,
        ...
        [HttpPost]
        public HttpResponseMessage Post([FromBody]ContractModel item)
        {
            _process.Save(item.Token, item.Name, item.Code,item.Observation, item.AvailableFrom, item.ContractClauses, item.IsActive, UpdatedId);
            return new HttpResponseMessage(HttpStatusCode.OK);
        }

        [HttpPut]
        public HttpResponseMessage Put(Guid id, [FromBody]ContractModel item)
        {
            _process.Save(item.Token, item.Name, item.Code, item.Observation, item.AvailableFrom, item.ContractClauses, item.IsActive, UpdatedId);
            return new HttpResponseMessage(HttpStatusCode.OK);
        }
