...
angular.module('App.Common.Cities', ['ngRoute', 'angular-loading-bar', 'ngAnimate', 'authService', 'breaDcrumb', 'serviceRest'])
    .config(['$routeProvider', function ($routeProvider) {
        $routeProvider.when("/Cities", {
            controller: "CitiesCtrl",
            templateUrl: "Partial/Common/Cities.html"
        });
        $routeProvider.when("/Cities/:Id", {
            controller: "CitieCtrl",
            templateUrl: "Partial/Common/Citiesdtl.html"
        });
    }])
    ...
    .controller('CitiesCtrl', ['$scope', 'serviceRest', 'breaDcrumb', 'serviceLocalized', 'authService', function ($scope, serviceRest, breaDcrumb, serviceLocalized, authService) {
        ...
        $scope.List = function (pageIndex) {
            ...
            $scope.promise = $scope.Rest.Get($scope.Settings.Uri, 'Cities2/Get?PageIndex=' + $scope.pagination.pageIndex + '&PageSize=' + $scope.pagination.pageSize + '&id=0', function (response) {
        ...
        $scope.Remove = function (id, status) {
            $scope.Rest.Patch($scope.Settings.Uri, 'Cities2/Remove/' + id + '?value=' + status, function (response) {
        ...
        $scope.Delete = function (id) {
            $scope.Rest.Delete($scope.Settings.Uri, 'Cities2/Delete/' + id, null, function (response) {
...
App.controller('CitieCtrl', ['$scope', 'serviceRest', 'breaDcrumb', '$routeParams', '$location', 'authService', function ($scope, serviceRest, breaDcrumb, $routeParams, $location, authService) {
    ...
    $scope.Get = function () {
        if ($routeParams.Id != 0) {
            $scope.Rest.Get($scope.Settings.Uri, 'Cities2/Get/' + $routeParams.Id, function (response) {
    ...
    $scope.Save = function () {
        ...
        if ($routeParams.Id == 0) {
            $scope.Rest.Post($scope.Settings.Uri, 'Cities2/Post', $scope.item,
                function (response) {
        ...
        if ($routeParams.Id != 0) {
            $scope.Rest.Put($scope.Settings.Uri, 'Cities2/Put/' + $routeParams.Id, $scope.item,
                function (response) {
